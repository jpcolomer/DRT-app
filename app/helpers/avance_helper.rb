module AvanceHelper
  ...
  def get_efectos(fecha)
    ...
    get_dotaciones.each do |dotacion|
      if x.get_date <= fecha
        recategorizacion += dotacion.recategorizacion
        gestion_dotacional += dotacion.gestion_dotacional
        nuevos_ingresos_egresos += dotacion.nuevos_ingresos_egresos
      end
    end
  ...
  def get_dotaciones_efectos
    ...
    dotacion_base = dotaciones.select{|x| x.get_date == fecha_base}.map{|x| x.empleados}.reduce(:+)
    dotacion_actual = dotaciones.select{|x| x.get_date == fecha_ultima}.map{|x| x.empleados}.reduce(:+)
    recategorizacion = 0
    gestion_dotacional = 0
    nuevos_ingresos_egresos = 0
    ...
    {:fecha_base => fecha_base, :fecha_ultima => fecha_ultima, :dotacion_base => dotacion_base, :dotacion_actual => dotacion_actual,:recategorizacion => recategorizacion, :gestion_dotacional => gestion_dotacional, :nuevos_ingresos_egresos => nuevos_ingresos_egresos}
  end
end